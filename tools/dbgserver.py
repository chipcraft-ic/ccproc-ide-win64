...
class GdbConn:
    ...
    def send_packet(self, data):
        ...
        while True:
            ...
            if not ack:
                ...
                logging.warning('Expected ACK from GDB, got \'%s\' (%s) (packet %s)',
                                ack, binascii.hexlify(ack).decode('ascii'), packet)
    ...
    def _prepare_packet(self, data):
        ...
        if is_py2 and type(data) is unicode:
            data = data.encode('ascii')
        for b in data:
            if type(b) is str:
                b = ord(b.encode('ascii'))
            if is_py2 and type(b) is unicode:
                b = ord(b.encode('ascii'))
            checksum += b
        checksum = checksum % 256
...
class DbgBridge:
    ...
    def cmd_write_mem_bin(self, arg):
        ...
        if len(data) != length:
            logging.error(
                'Invalid length field in write_mem_bin command %d %d', len(data), length)
    ...
    def get_exception_info(self):
        ...
        # current core
        repl += 'thread:{0:08X};'.format(self._cpu_dbg.get_core() + 1)
